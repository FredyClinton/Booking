...
public class BookingServiceImpl implements BookingService {
    ...
    public CreateBookingResponseDTO createBooking(CreateBookingRequestDTO dto, String username) {
        ...
        createdBooking.setAddresse(dto.getAddresse());
    ...
    public CreateBookingResponseDTO updateBookingFull(UUID id, UpdateBookingRequestDTO dto, String username) {
        ...
        Booking booking = bookingRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Booking not found"));
        boolean isOwner = booking.getUser() != null && booking.getUser().getEmail() != null && booking.getUser().getEmail().equals(SecurityUtils.getCurrentUsername());
        if (!isOwner) {
    ...
    @Override
    public List<CreateBookingResponseDTO> getAllBookings() {
        UserEntity user = SecurityUtils.getCurrentUser();

        List<Booking> bookings;
        if (user != null && SecurityUtils.currentUserHasRole("ROLE_ADMIN")) {
            bookings = bookingRepository.findAll();
        } else if (user != null && SecurityUtils.currentUserHasRole("ROLE_EMPLOYEE")) {
            bookings = bookingRepository.findDistinctByEmployees_Email(user.getEmail());
        } else {
            bookings = bookingRepository.findByUser_Email(SecurityUtils.getCurrentUsername());
        }
    ...
    public CreateBookingResponseDTO requestDeletion(UUID id) {
        UserEntity user = SecurityUtils.getCurrentUser();
        Booking booking = bookingRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Booking not found"));
        if (user != null && (SecurityUtils.currentUserHasRole("ROLE_EMPLOYEE") || !isAssignedToEmployee(booking, user.getEmail()))) {
